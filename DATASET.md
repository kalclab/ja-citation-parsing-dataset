...
| フラグ | 意味 | 件数 |
|---|---|---|
| `nonstandard_volume` | 通巻表記等の非標準な巻号 | 219 |
...
各ソースは API またはエクスポートを優先して取得し、リクエスト間に 1〜2 秒の待機を置いた。robots.txt と利用条件の確認結果は `docs/collection_compliance.md` に記録している。
...
- **巻号の復号**: CiNii の `123(6)` 形式は volume=123 / number=6 に分解する（同一情報の復号であり改変ではない）。真に非標準な巻号（`125巻5号`・通巻表記等）はソース表記のまま保持し `nonstandard_volume` を付与する。
...
| スタイル | 実装 | 想定分野 |
|---|---|---|
| jecon-mod | .bst | 経済学（人文社会）。jecon ver.6.6 の 1 トークン改変版（和文会議録名を出力）。`scripts/README.md` 参照 |
...
スタイル×文献タイプごとに、どのフィールドが文字列に現れるかの実測対応表は `reports/style_field_matrix.md` にある。
...
- 生成後の突き合わせ（スタイルごとのランダム 20 件目視照合・スタイル間の出力同一率）は `reports/refs_verification.md` に記録している。
